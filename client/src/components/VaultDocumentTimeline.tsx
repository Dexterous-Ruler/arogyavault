    ...
    >
      <div className="flex flex-col bg-white border-b border-gray-200 sticky top-0 z-20" data-testid="vault-header">
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <h1 className="text-xl font-semibold text-gray-900" data-testid="text-vault-title">{t.title}</h1>
          <button 
        ...
        <div className="overflow-x-auto scrollbar-hide px-6 pb-4" data-testid="filter-container">
            ...
            {filterOptions.map(option => (
              ...
              >
                ...
                <span className="text-sm font-medium">
                  {option.id === 'all' ? t.all : 
                   option.id === 'prescription' ? t.prescriptions : 
                   option.id === 'lab' ? t.labReports : 
                   option.id === 'imaging' ? t.imaging : 
                   t.billing}
                </span>
              </button>
            ))}
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4" data-testid="documents-container">
        {filteredDocuments.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full py-12" data-testid="empty-state">
            <div className="w-32 h-32 mb-6 rounded-full bg-gray-100 flex items-center justify-center">
              <FileText className="w-16 h-16 text-gray-400" />
            </div>
            <p className="text-lg font-medium text-gray-900 mb-2" data-testid="text-empty-state">{t.emptyState}</p>
            <button 
            ...
            {filteredDocuments.map(doc => (
              ...
              >
                <div className="flex gap-3">
                  <div className="flex-shrink-0 w-14 h-14 rounded-lg bg-gray-100 flex items-center justify-center" data-testid={`icon-document-${doc.id}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-600 mb-2" data-testid={`text-document-date-${doc.id}`}>{doc.date}</p>
      ...
      {offlineCount > 0 && (
        ...
        >
          ...
          <span className="text-sm font-medium text-orange-900" data-testid="text-offline-count">
      ...
      <button 
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-blue-600 rounded-full shadow-lg flex items-center justify-center hover:bg-blue-700 transition-all hover:scale-105 z-30" 
        style={{
        ...
        {showAddMenu && (
          <>
            ...
            <motion.div 
              ...
              className="fixed bottom-24 right-6 bg-white rounded-xl shadow-2xl overflow-hidden z-50" 
              style={{
                right: 'calc(50% - 195px + 24px)',
                minWidth: '200px'
              }}
            ...
            >
              {addMenuOptions.map((option, idx) => (
                <button 
                  ...
                  className="w-full px-5 py-4 flex items-center gap-3 hover:bg-gray-50 transition-colors text-left border-b border-gray-100 last:border-b-0"
                  data-testid={`button-add-${idx === 0 ? 'camera' : idx === 1 ? 'upload' : idx === 2 ? 'qr' : 'dicom'}`}
                >
